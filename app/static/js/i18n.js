...
const I18N = {
  en: {
    ...
    restore: {
      ...
      desc: 'Upload the backup zip. The wizard auto-fixes TimescaleDB version and password/SASL issues when possible.',
      ...
      targetDb: 'Final database',
      targetHint: 'If installed DB differs from the backup, conversion runs automatically when possible.',
      experimentalBadge: 'Auto-convert',
      experimentalLabel: 'I confirm the backup DB differs from the installed one and conversion should run',
      needExperimental: 'Confirm conversion to continue with a different database type',
  ...
  fa: {
    ...
    restore: {
      ...
      desc: 'فایل zip بکاپ را آپلود کنید. در صورت نیاز، نسخه TimescaleDB و مشکل پسورد/SASL به‌صورت خودکار درست می‌شود.',
      ...
      targetDb: 'دیتابیس نهایی',
      targetHint: 'اگر دیتابیس نصب با بکاپ فرق دارد، تبدیل خودکار انجام می‌شود. در صورت نیاز تیک زیر را بزنید.',
      experimentalBadge: 'تبدیل خودکار',
      experimentalLabel: 'تأیید می‌کنم دیتابیس بکاپ با نصب فعلی فرق دارد و تبدیل انجام شود',
      needExperimental: 'برای تغییر نوع دیتابیس، تیک آزمایشی را بزنید',
  ...
  ru: {
    ...
    restore: {
      ...
      desc: 'Загрузите zip. Мастер по возможности исправит версию TimescaleDB и ошибки пароля/SASL.',
      ...
      targetDb: 'Итоговая БД',
      targetHint: 'Смена типа БД экспериментальна. Лучше оставить ту же СУБД, что в бэкапе.',
      experimentalBadge: 'Экспериментально',
      experimentalLabel: 'Понимаю, что смена БД экспериментальна и может завершиться ошибкой',
      needExperimental: 'Отметьте экспериментальный пункт для смены типа БД',
