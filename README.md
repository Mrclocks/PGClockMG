...
> ⚠️ **`v3.2.6`** — قبل از ریستور یا مهاجرت حتماً بکاپ کامل بگیرید.
...
ویزارد از `v3.2.6` با **توکن دسترسی** محافظت می‌شود. بدون توکن کسی به پنل دسترسی ندارد.
نصب‌کننده در پایان یک URL آماده با توکن چاپ می‌کند. در صورت نیاز:
...
از `v3.2.6` ویزارد بدون توکن باز نمی‌شود. بعد از نصب، همان URL چاپ‌شده توسط
...
توکن در فایل `/opt/pg-migrator/.access_token` با دسترسی `0600` ذخیره می‌شود.
