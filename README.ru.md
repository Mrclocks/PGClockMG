> ⚠️ **`v3.2.6`** — Перед restore или миграцией сделайте полный бэкап.
...
С `v3.2.6` мастер защищён **токеном доступа**. Без него панель недоступна.
Установщик в конце печатает готовую ссылку с токеном. Если нужна позже:
...
С `v3.2.6` мастер без токена не открывается. После установки используйте URL,
...
Токен хранится в `/opt/pg-migrator/.access_token` с правами `0600`.
